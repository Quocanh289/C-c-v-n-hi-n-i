...
const emptyAnalysis: AggregatedAnalysis = {
  ...
  risk_level: 'Low',
  ai_summary:
    'Chua co du du lieu da luu de tao xu huong ca nhan. Emotion Lens chi cung cap thong tin tham khao, khong thay the chan doan y khoa.',
};
...
function issueTone(issue?: string | null) {
  ...
  if (normalized.includes('stress')) {
    return {
      ...
      badge: 'bg-orange-500/10 text-orange-100 ring-orange-300/40',
    };
  }
  if (normalized.includes('anxiety') || normalized.includes('anxious')) {
    return {
      ...
      badge: 'bg-yellow-500/10 text-yellow-100 ring-yellow-300/40',
    };
  }
  if (normalized.includes('depression')) {
    return {
      ...
      text: 'text-blue-300',
      badge: 'bg-blue-500/10 text-blue-100 ring-blue-300/40',
    };
  }
  if (normalized.includes('suicidal')) {
    return {
      ...
      text: 'text-red-300',
      badge: 'bg-red-500/15 text-red-100 ring-red-300/45',
    };
  }
  if (normalized.includes('bipolar')) {
    return {
      ...
      badge: 'bg-violet-500/10 text-violet-100 ring-violet-300/40',
    };
  }
  if (normalized.includes('personality')) {
    return {
      ...
      text: 'text-pink-300',
      badge: 'bg-pink-500/10 text-pink-100 ring-pink-300/40',
    };
  }
  if (normalized.includes('normal')) {
    return {
      ...
      text: 'text-slate-300',
      badge: 'bg-slate-700/70 text-slate-100 ring-slate-500/45',
    };
  }
  ...
  return {
    ...
    text: 'text-slate-300',
    badge: 'bg-slate-800 text-slate-200 ring-slate-500/40',
  };
}
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          ...
          {sourceUrl ? (
            ...
            <p className="mt-1 text-xs text-slate-500">Khong co source URL</p>
          )}
        </div>
    ...
    return (
      <main className="min-h-screen bg-slate-950 px-6 py-16 text-slate-100">
        <section className="mx-auto max-w-3xl rounded-lg border border-red-500/30 bg-red-500/10 p-6 text-center">
          <p className="text-lg font-semibold text-red-200">Thieu User ID</p>
          <p className="mt-2 text-sm text-red-100/80">
            Vui long mo dashboard tu popup Emotion Lens de he thong truyen dung ma nguoi dung.
          </p>
        </section>
      </main>
    );
    ...
    return (
      <main className="min-h-screen bg-slate-950 px-6 py-16 text-slate-100">
        <section className="mx-auto max-w-3xl rounded-lg border border-slate-800 bg-slate-900 p-6 text-center">
          <p className="text-lg font-semibold text-slate-100">Khong tai duoc dashboard</p>
          <p className="mt-2 text-sm text-slate-400">
            Backend chua phan hoi hoac UID khong hop le. Hay kiem tra FastAPI tai localhost:8000.
          </p>
        </section>
      </main>
    );
            ...
            <div className="rounded-lg border border-slate-800 bg-slate-900/70 p-5 shadow-xl shadow-slate-950/30">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500">Lich su van ban da luu, moi nhat o tren cung.</p>
                </div>
              ...
              {savedTexts.length > 0 ? (
                ...
                <div className="mt-5 rounded-lg border border-slate-800 bg-slate-900 p-8 text-center">
                  <p className="text-base font-semibold text-slate-200">Chua co snippet nao duoc luu</p>
                  <p className="mt-2 text-sm text-slate-500">
                    Khi ban luu van ban tu Chrome Extension, du lieu se xuat hien tai day.
                  </p>
                </div>
              )}
            </div>
          ...
          <aside className="lg:col-span-4 lg:sticky lg:top-6">
            <section className="relative overflow-hidden rounded-xl border-2 border-indigo-300/45 bg-gradient-to-br from-indigo-500/20 via-violet-500/15 to-slate-900 p-6 shadow-[0_0_50px_rgba(99,102,241,0.35)]">
              ...
              <div className="relative z-10">
                ...
                <p className="mt-5 text-sm leading-6 text-indigo-50/95">{analysis.ai_summary}</p>

                <button
                  type="button"
                  className="mt-6 inline-flex w-full items-center justify-center rounded-md bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-500"
                >
                  Keep Monitoring
                </button>
              </div>
